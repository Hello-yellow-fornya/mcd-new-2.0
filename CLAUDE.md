...
- **Indexing (2.0 only):** the homepage is indexable on the live domain; every other page (content, landing, utility) is `noindex, nofollow` everywhere, because 1.0 carries the same copy. Per §2a this keys off the build's canonical origin, not `VERCEL_ENV`, so staging and previews index nothing: `src/lib/indexing.ts` holds the indexable set, and the meta tag, the `X-Robots-Tag` header and `robots.txt` all read it. No `sitemap.xml`. Canonicals point to self on the 24/7 URL, never to 1.0. A test walks every route for this and for links to the 1.0 domain.
...
- **Scope [assumption]:** Phase 1 is the homepage, the two landing pages, claim-now (the reg box and the claim flow), about, contact and the legal set. The SEO template set is **not** replicated for 2.0 unless told; stub the sitemap routes as drafts.
- **Backend:** no `api/` in this repo. Post to the 1.0 claims API with `source: "mcd2"`, using `CLAIMS_API_URL` and a 2.0-specific `CLAIMS_API_KEY`. Claim-now renders the claim flow at `#claim-flow`: 1.0's questions and branches, duplicated in `src/lib/claim-flow/` and skinned in 2.0, posting to `/api/claim-submit/`.
