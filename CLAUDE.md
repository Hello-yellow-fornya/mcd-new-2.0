...
- **Header (replace §4 SiteHeader):** sticky cream bar, 84px desktop: logo hard left at 50px (the wide Claims 24/7 lockup, §4a); sections How it works · Non-fault accident · Services ▾ · Advice · About with a yellow underline on the active page; a white "Lines open 24/7 · Avg wait 1 min" chip (substantiation-gated); a yellow phone-number pill and an ink "Start your claim" pill hard right. Mobile 64px: logo left at 42px, ink "Call now" pill and burger hard right, nothing else; the proof line lives in the drawer.
...
Sources in `design/logo/`, the suite as delivered: six lockups, nine squares and the favicon set. The wordmark is `<text>` in those files on purpose so the words stay editable; `scripts/logo-build.mjs` (prebuild, `pnpm logo`) converts every wordmark to outlines with the self-hosted Archivo Black, writing the build output to `public/logo/`, so the type never falls back to another face.
...
- **Squares** in `design/logo/square/`: `claims247-square-mark-on-*`, `-lines-on-*` and `-stacked-on-*` (mark over "Claims" over "24/7"), each on yellow, ink and white. The stacked square on yellow is the social avatar (`pnpm logo:png` cuts the PNG).
- **Favicons** in `design/logo/favicons/`, as delivered: `favicon-source.svg` (the mark on yellow) and PNGs at 16, 32, 48, 180, 192, 512 and 1024. `pnpm logo` copies them to `public/favicons/`, `src/app/icon.svg` and `src/app/apple-icon.png`; the manifest points at the 192, 512 and 1024 PNGs.
- **Wide arrangement** (`claims247-logo-wide-*`, built from the same sources): the wordmark scaled to the mark's full height and set beside the stopwatch with a gap, not tucked into its opening. This is the header and footer lockup: the signed-off compact arrangement sets "Claims" too small at header heights.
- **Component:** `<Logo surface="light | cream | yellow | ink | mono-ink | mono-white" />` picks the variant by surface and renders the wide lockup inline from `lockup.generated.tsx` (`<Lockup layout="compact">` for the signed-off arrangement). **Header:** on-cream (on-light on a white surface) at 50px tall desktop, 42px mobile, the MOTOR lockup's footprint. **Footer:** on-ink.
